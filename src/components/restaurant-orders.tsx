...
function OrderCard({ row, restaurantId, riders, history = false }: { row: OrderRow; restaurantId: string; riders: Rider[]; history?: boolean }) {
  return <article className="rounded-lg border p-4"><div className="flex flex-col gap-2 sm:flex-row sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">#{row.order.orderNumber}</h3><span className="rounded-full bg-gray-100 px-2 py-1 text-xs font-medium">{labels[row.status] ?? row.status}</span>{!history && <span className="rounded-full border px-2 py-1 text-xs text-gray-600">Running <OrderElapsedTime createdAt={row.createdAt.toISOString()}/></span>}</div><p className="mt-1 text-sm text-gray-600">{row.order.customer.firstName} {row.order.customer.lastName} · {row.order.customer.email}</p></div><div className="text-sm sm:text-right"><p className="font-medium">₦{Number(row.subtotal).toLocaleString()}</p><p className="text-xs text-gray-500">Restaurant subtotal · {row.createdAt.toLocaleString()}</p></div></div>
    ...
    {!history && <div className="mt-4 flex flex-wrap gap-2">
      ...
      {row.status === "OUT_FOR_DELIVERY" && <form action={assignRider} className="flex flex-wrap gap-2"><Hidden restaurantId={restaurantId} orderId={row.id}/><select name="riderId" required defaultValue={row.delivery?.riderId ?? ""} className="rounded-lg border px-3 py-2 text-sm"><option value="" disabled>Select active rider</option>{riders.map(r => <option key={r.userId} value={r.userId}>{r.user.firstName || r.user.email} {r.user.lastName}</option>)}</select><button className={action}>{row.delivery?.riderId ? "Change rider" : "Assign rider"}</button>{riders.length === 0 && <p className="basis-full text-xs text-gray-500">No active riders are attached to this restaurant.</p>}</form>}
