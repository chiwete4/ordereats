...
function personLabel(person: { firstName: string; lastName: string; email: string }) {
...
function OrderCard({ row, restaurantId, riders, history = false }: { row: OrderRow; restaurantId: string; riders: Rider[]; history?: boolean }) {
  return <article className="rounded-lg border p-4"><div className="flex flex-col gap-2 sm:flex-row sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">#{row.order.orderNumber}</h3><span className="rounded-full bg-gray-100 px-2 py-1 text-xs font-medium">{labels[row.status] ?? row.status}</span>{!history && <span className="rounded-full border px-2 py-1 text-xs text-gray-600">Running <OrderElapsedTime createdAt={row.createdAt.toISOString()}/></span>}</div><p className="mt-1 text-sm text-gray-600">{personLabel(row.order.customer)} · {row.order.customer.email}</p></div><div className="text-sm sm:text-right"><p className="font-medium">₦{Number(row.subtotal).toLocaleString()}</p><p className="text-xs text-gray-500">Restaurant subtotal · {row.createdAt.toLocaleString()}</p></div></div>
    ...
    {row.delivery && <div className="mt-3 rounded-lg bg-gray-50 p-3 text-sm"><p><strong>Delivery:</strong> {row.delivery.status.replaceAll("_", " ").toLowerCase()}</p><p><strong>Rider:</strong> {row.delivery.rider ? personLabel(row.delivery.rider) : "Not assigned"}</p></div>}
